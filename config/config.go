...
type Config struct {
	...
	// 豆瓣榜单相关配置
	DoubanFetchInterval time.Duration // 豆瓣榜单预抓取周期
	DoubanHTTPTimeout   time.Duration // 豆瓣接口请求超时
	DoubanCacheTTL      time.Duration // 豆瓣榜单缓存有效期
...
func Init() {
	...
	AppConfig = &Config{
		...
		DoubanFetchInterval: getDoubanFetchInterval(),
		DoubanHTTPTimeout:   getDoubanHTTPTimeout(),
		DoubanCacheTTL:      getDoubanCacheTTL(),
	}
...
// 从环境变量获取豆瓣榜单预抓取周期（分钟），默认30分钟
func getDoubanFetchInterval() time.Duration {
	value := os.Getenv("DOUBAN_FETCH_INTERVAL_MINUTES")
	if value == "" {
...
// 从环境变量获取豆瓣接口请求超时（秒），默认10秒
func getDoubanHTTPTimeout() time.Duration {
	value := os.Getenv("DOUBAN_HTTP_TIMEOUT_SECONDS")
	if value == "" {
...
// 从环境变量获取豆瓣榜单缓存有效期（分钟），默认2880分钟（48小时）
func getDoubanCacheTTL() time.Duration {
	value := os.Getenv("DOUBAN_CACHE_TTL_MINUTES")
	if value == "" {
		return 2880 * time.Minute
	}
	minutes, err := strconv.Atoi(value)
	if err != nil || minutes <= 0 {
		return 2880 * time.Minute
